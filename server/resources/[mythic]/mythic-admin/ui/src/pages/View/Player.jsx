...
export default ({ match }) => {
	...
	const onAction = async (action) => {
		try {
			let res;
			if (action === 'OpenPlyInv') {
				res = await (await Nui.send('OpenPlyInv', {
					targetSource: player?.Source,
					action: action,
				})).json();
			} else {
				res = await (await Nui.send('ActionPlayer', {
					targetSource: player?.Source,
					action: action,
				})).json();
			}
					...
					<Grid className={classes.wrapper} container spacing={2}>
						<Grid item xs={12}>
							<ButtonGroup fullWidth variant="contained">
								...
								{/* {(permissionLevel >= 80) && <Button onClick={() => onAction('OpenPlyInv')} disabled={!player?.Character || player.Disconnected}>
									Open inv
								</Button>} */}
								<Button onClick={onRefresh}>
									Refresh
								</Button>
								{(permissionLevel >= 0) && <Button onClick={() => onAction('marker')} disabled={(user?.Source === player.Source)}>
									GPS Marker
								</Button>}
							</ButtonGroup>
						</Grid>
								...
								<>
									<List>
										...
										<ListItem>
											<ListItemText
												primary="DOB"
												secondary={moment(player.Character.DOB).format('LL')}
											/>
										</ListItem>
										{permissionLevel >= 0 && (<ListItem>
											<ListItemText
