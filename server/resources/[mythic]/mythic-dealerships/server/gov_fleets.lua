        ...
        if chosenVehicle and chosenVehicle.liveries[livery] ~= nil then
            ...
            if purchaseBankAccount and purchaseBankAccount.Account then
                if Banking.Balance:Charge(purchaseBankAccount.Account, chosenVehicle.price, {
                    ...
                    Vehicles.Owned:AddToFleet(
                        ...
                        function(success, vehicle)
                            if success and vehicle then
                                Execute:Client(src, 'Notification', 'Success', string.format('Fleet Vehicle Purchase of a %s %s was Successful.<br><br>VIN: %s<br>Plate: %s', chosenVehicle.make, chosenVehicle.model, vehicle.VIN, vehicle.RegisteredPlate), 5000)
                            else
                    ...
                    Execute:Client(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed - Not Enough Money in the Bank', 5000)
                end
            else
                Execute:Client(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed', 5000)
            end
        else
            Execute:Client(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed - Invalid Vehicle', 5000)
        end
    else
        Execute:Client(src, 'Notification', 'Error', 'Fleet Vehicle Purchase Failed', 5000)
    end
