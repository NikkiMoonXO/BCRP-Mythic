...
AddEventHandler("Core:Shared:Ready", function()
	exports["mythic-base"]:RequestDependencies("Tow", {
		...
		}, "truck-pickup", "WORLD_HUMAN_HANG_OUT_STREET")
	...
	if not _towingAction and _towTrucks[truckModel] and truckState and not truckState.towingVehicle then
		...
		if canTow then
			...
			Progress:ProgressWithStartAndTick({
				...
				if not canTow then
					Progress:Cancel()
					Notification:Error(errorMessage, 5000, "truck-pickup")
				end
				...
				if not wasCancelled then
					...
					if success then
						...
						Notification:Success("Vehicle Now on Tow Truck", 5000, "truck-pickup")
						...
						Notification:Error("Failed to Tow Vehicle", 5000, "truck-pickup")
					end
				end
			end)
		else
...
AddEventHandler("Vehicles:Client:ReleaseTow", function(entityData)
	...
	if _towTrucks[truckModel] and truckState then
		if truckState.towingVehicle then
			...
			if success then
				Notification:Success("Vehicle Released from Truck", 5000, "truck-pickup")
				truckState:set("towingVehicle", false, true)
			end
		else
			Notification:Error("No Vehicle Being Towed", 5000, "truck-pickup")
		end
	end
end)
