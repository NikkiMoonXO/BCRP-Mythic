...
export default ({ alert }) => {
	...
	const getIcon = () => {
		switch (alert.style) {
			case 1:
				return <FontAwesomeIcon icon={['fas', 'clipboard-list']} />;
			case 2:
						...
						<div className={classes.minor2}>
							...
							{hasAttaching && <div>
								<IconButton onClick={() => {
									...
									<FontAwesomeIcon style={{ fontSize: 15, margin: 'auto' }} icon={['fas', meAttached ? 'link-slash' : 'link-slash']} />
								</IconButton>
							</div>}
							{hasAttaching && <div>
								<Autocomplete
						...
						<Grid item xs={1} style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignContent: 'right' }}>
							...
							{Boolean(alert.location) && (
								<IconButton onClick={onRoute} style={{ marginLeft: 'auto' }}>
									<FontAwesomeIcon size="xs" icon={['fas', 'location-dot']} />
								</IconButton>
							)}
