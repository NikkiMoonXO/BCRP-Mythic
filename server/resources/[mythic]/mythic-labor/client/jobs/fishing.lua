...
RegisterNetEvent("Fishing:Client:StartFishing", function(toolUsed)
    ...
    if fishingZone == 4 or fishingZone == 5 then
        Notification.Persistent:Info("fishing-info-notif", string.format("Fishing - Press %s to Stop. Maybe you could try some deeper water for better fish!", Keybinds:GetKey("cancel_action")), "fishing-fins")
    else
        Notification.Persistent:Info("fishing-info-notif", string.format("Fishing - Press %s to Stop", Keybinds:GetKey("cancel_action")), "fishing-fins")
    end
...
function DoFishBite(zone, toolUsed)
    ...
    Callbacks:ServerCallback("Fishing:Catch", {
        ...
        if not success and not stopFishing then
            Notification:Warn("The Fish Got Away...", 5000, "fishing-fins")
        end

        if stopFishing then
            print("Stopping the Fishing")
            _isFishing = false
        end
    end)
...
function GenerateNextFishTime(zone, toolUsed)
    ...
    if zone == 4 or zone == 5 then -- Add more time for these
        minTime += 60
    elseif zone > 6 then
        minTime += 30
    end
