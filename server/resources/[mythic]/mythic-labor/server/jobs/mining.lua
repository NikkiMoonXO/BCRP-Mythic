...
local function GenerateNodes(source)
	...
	if not antiDouche[source] then
		...
		if ultra == 1000000 then
			...
			local repChance = _repHEChance[calcLvl] or 0
			if luck == 100 then
				heCount = 2
			elseif luck >= (100 - repChance) then
				heCount = 1
			else
				heCount = 0
			end
...
AddEventHandler("Labor:Server:Startup", function()
	...
	Callbacks:RegisterServerCallback("Mining:Server:MineNode", function(source, data, cb)
		local char = Fetch:CharacterSource(source)
		-- Check for Diamond Brew effect (simple inventory check, can be replaced with buff system if available)
		local hasDiamondBrew = Inventory.Items:GetCount(char:GetData("SID"), 1, "special_mining_coffee") > 0
		local diamondChanceBoost = 0
		local miningSpeedBoost = 0
		if hasDiamondBrew then
			diamondChanceBoost = 25 -- percent
			miningSpeedBoost = 0.5 -- 50% faster
		end
				...
				for i = #_mining[_joiners[source]].nodes, 1, -1 do
					...
					if node then
						...
						then
							...
							local luck = math.random(100)
							if hasDiamondBrew and node.ore.item == "diamond" then
								luck = luck + diamondChanceBoost
							end
